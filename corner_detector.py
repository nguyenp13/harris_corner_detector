...
def main():
    ...
    denoising_sigma = float(get_command_line_param_val_default_value(sys.argv,'-denoising_sigma',2)) # the sigma we use for our initial gaussian denoising of our image
    neighborhood_width = float(get_command_line_param_val_default_value(sys.argv,'-neighborhood_width',5)) # how big of a neighborhood we want to check for corners
    threshold = float(get_command_line_param_val_default_value(sys.argv,'-threshold',15000)) # the threshold we want to use to make sure our our eigenalues are large enough
    
    print "Parameters: "
    print "    Input Image Location:",input_image_location
    ...
    kernel_dim = int(math.floor(denoising_sigma*3/2.0)*2+1) # we multiply by three to get about a 99% confidence interval
